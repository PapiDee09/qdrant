...
use super::{Backend, query1bit, query2bit, query4bit};
...
impl<const PLANES: usize, const QUERY_BYTES: usize> QueryPlanes<PLANES, QUERY_BYTES> {
    /// Build the planes for a `dim` that is a multiple of `PLANES`;
    /// `encode(j)` yields the bytes of query dim `j`.
    fn new(dim: usize, mut encode: impl FnMut(usize) -> [i8; QUERY_BYTES]) -> Self {
        debug_assert!(dim.is_multiple_of(PLANES));
        let len = (dim / PLANES).next_multiple_of(PLANE_BLOCK);
        let encoded: Vec<[i8; QUERY_BYTES]> = (0..dim).map(&mut encode).collect();
        let mut bytes: [[Vec<i8>; PLANES]; QUERY_BYTES] =
            std::array::from_fn(|_| std::array::from_fn(|_| vec![0; len]));
        for (b, byte_planes) in bytes.iter_mut().enumerate() {
            for (k, plane) in byte_planes.iter_mut().enumerate() {
                for (entry, dims) in plane.iter_mut().zip(encoded.chunks_exact(PLANES)) {
                    *entry = dims[k][b];
                }
            }
        }
...
impl<const PLANES: usize, const QUERY_BYTES: usize> QuerySimd<PLANES, QUERY_BYTES> {
    ...
    pub fn new(data: &[f32]) -> Self {
        ...
        let planes = QueryPlanes::new(data.len(), |j| {
            let q_signed = (data[j] * q_scale).round().clamp(clamp_lo, clamp_hi) as i32;
