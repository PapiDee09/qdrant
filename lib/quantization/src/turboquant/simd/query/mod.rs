...
impl<const PLANES: usize> QueryPlanes<PLANES> {
    ...
    /// `encode(j)` yields the `(low, high)` halves of query dim `j`.
    fn new(dim: usize, mut encode: impl FnMut(usize) -> (i8, i8)) -> Self {
        ...
        for j in 0..dim / PLANES {
            for k in 0..PLANES {
                let (l, h) = encode(PLANES * j + k);
                low[k][j] = l;
                high[k][j] = h;
            }
        }
...
impl<const PLANES: usize> QuerySimd<PLANES> {
    ...
    fn dotprod_raw_best(&self, vector: &[u8]) -> i64 {
        match self.backend {
            ...
            Backend::Avx512Vnni | Backend::Avx2 | Backend::Sse => self.dotprod_raw(vector),
            #[cfg(all(target_arch = "aarch64", target_feature = "neon"))]
