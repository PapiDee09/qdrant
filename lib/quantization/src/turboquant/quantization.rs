...
impl TurboQuantizer {
    ...
    /// of encoded vectors: scores the vector at `data[v * quantized_size()..]`
    /// into `scores[v]`.  The width's kernel scores the whole run in one call
    /// (see [`crate::turboquant::simd::QuerySimd::dotprod_batch`]), then the
    /// extras are applied per vector.
    ///
    /// # Panics
    /// Panics if `data` holds fewer than `scores.len()` encoded vectors.
    pub fn score_precomputed_batch(&self, query: &EncodedQueryTQ, data: &[u8], scores: &mut [f32]) {
        ...
        let codes_len = stride - extras_len;
